...
import { getTubeUsage, recordUsage, updateUsageEntry, deleteUsageEntry, setUsageRef } from '../api/tube_usage'
...
export const useUpdateUsageEntry = (specimenId: number) => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ entryId, data }: { entryId: number; data: TubeUsageLogCreate }) =>
      updateUsageEntry(specimenId, entryId, data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tube-usage', specimenId] })
      qc.invalidateQueries({ queryKey: ['specimen', specimenId] })
    },
